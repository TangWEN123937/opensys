...
- **涉及网站浏览器操作（登录、采集数据、操作动态页面）的阶段必须使用 method: browser**
...
{{
  ...
  "phases": [
    {{
      ...
      "method": "agent/browser/executor/executor_parallel/reviewer",
      "skill": "技能ID或null",
      "required": true,
      "review": true,
...
async def advisor_node(state: dict) -> dict:
    ...
    advisor_llm = get_llm(_advisor_model)
    messages = [
    ...
    if action == "approved":
        ...
        if _mode == "adjust":
            ...
            todos = _pipeline_to_todos(pipeline)
        ...
        # === 全量模式（首次规划 / replan）===
        # 用户确认 → pipeline 写入 State + 转为 todos
...
def _fix_browser_method(pipeline: dict) -> list[str]:
    ...
    for phase in phases:
        ...
        desc = (phase.get("description", "") + " " + phase.get("details", "")).lower()
...
def _pipeline_to_todos(pipeline: dict) -> list[dict]:
    """
    将 pipeline 的 phases 转换为 todos 列表

    pipeline 是战略层（骨架），todos 是战术层（具体步骤）。
    每个 phase 生成一个 todo，Agent 执行过程中可以进一步细化。
    ...
    for i, phase in enumerate(phases):
        todo = {
            "id": str(phase.get("id", i + 1)),
            "content": f"[Phase {phase.get('id', i + 1)}] {phase.get('name', '未知')}: {phase.get('description', '')}",
            "status": "in_progress" if i == 0 else "pending",
            "priority": "high" if phase.get("required", True) else "medium",
        }
        todos.append(todo)
